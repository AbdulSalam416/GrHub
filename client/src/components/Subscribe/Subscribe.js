...
const Subscribe = () => {
  const images = setPictures(
    require.context(
      "../../assets/img/c-subscribe/",
      false,
      /\.(png|jpe?g|svg|webp)$/
    )
  );
  ...
  const subscribeUser = async (event) => {
    ...
    try {
      let gg = await axios.post("/api/subscribeRoute/subscribe", {
        email: email,
      });
      notify();
      ...
      <div className="c-subscribe">
        ...
        <div className="c-subscribe-box">
          <div className="c-subscribe-box-inner">
            <h2>Curious? Get in touch and be in the know!</h2>
          </div>
